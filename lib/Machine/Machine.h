/*
  Copied from https://github.com/m-elias/AOG-Machine
    Feb 2025
  ...
  This class only takes care of the AgOpenGPS "Machine" logic & PGN parsing
    ...
    - AiO onboard Section Controls scan reply can be switched OFF by setting Pin24 to section 16 in AgOpenGPS' config. Needed if external section control is used.
...
public:
  ...
  struct States
  {
    ...
    uint8_t SecFromAOG[2]; // section 1-16 on/off command from AgopenGPS, Machine Data PGN, [0] = sect 7-0 (bitweise) [1] = sect 16-9
    // uint8_t sec1to8;              // section 1 to 8 from Machine Data PGN
    // uint8_t sec9to16;             // section 9 to 16 from Machine Data PGN
    bool SecContOnBoardIsActive; // true if the section control is on AiO board, false if it is on separate board (ie. in the implemet)
...
#define EE_IDENT 11    // change to force EE to reset to defaults
  ...
  const uint16_t watchdogTimeoutPeriod = 10000;//5000; // ms, originally was 20 update cycles (4 secs)
  const uint16_t watchdogAlertPeriod = 5000;//2000;   // ms, how long after UDP comms lost to alert to possible comms issues
...
public:
  ...
  void watchdogCheck()
  {
    if (watchdogTimer > watchdogTimeoutPeriod) // watchdogTimer reset with Machine Data PGN, should be 64 Section instead or both?
    {
      ...
      if (MachineOutputs_Handler != NULL)
        MachineOutputs_Handler(); // callback function to update machine outputs (incl sections 1-16)
      if (SectionOutputs_Handler != NULL)
        SectionOutputs_Handler(); // callback function to update section only outputs
  ...
  void updateMachineStates()
  {
    ...
    if (config.hydLiftEnable) // hydraulic lift
    {
      ...
      if (states.hydLift != 1 && states.hydLift != 2)
      { //|| gpsSpeed < 2)
        lowerTimer = 0;
        raiseTimer = 0;
        lastTrigger = 0;
      ...
      if (lowerTimer)
        isLower = true; // if lower is active, set ON
      if (raiseTimer)
        isRaise = true; // if raise is active, set ON
    }
    else
    { // hyd lift is disabled, make sure any hyd lift pins are OFF
      isLower = false;
      isRaise = false;
    }
  ...
  bool parsePGN(uint8_t *pgnData, uint8_t len, IPAddress sourceIP, IPAddress myIP)
  {
    ...
    if (pgnData[0] != 0x80 || pgnData[1] != 0x81 || pgnData[2] != 0x7F)
      return false; // skip the rest if the first three bytes are NOT AoG headers

    if (pgnData[3] == 200 && len == 9) // 0xC8 (200) - Hello from AgIO
    {
      if (debugLevel > 3)
        printPgnAnnoucement(pgnData, len, (char *)"Hello from AgIO");

      // if (isInit) {
      if (states.SecContOnBoardIsActive)
      {
        ...
        if (UDPReplyHandler != NULL)
        {
          ...
          if (debugLevel > 3)
            printPgnAnnoucement(helloFromMachine, sizeof(helloFromMachine), (char *)"Machine Reply");
        }
      }
      else
      {
    ...
    {
      if (debugLevel > 2)
        printPgnAnnoucement(pgnData, len, (char *)"Scan Request");
      ...
      if (states.SecContOnBoardIsActive)
      {
        if (pgnData[4] == 3 && pgnData[5] == 202 && pgnData[6] == 202)
        {
          ...
          if (UDPReplyHandler != NULL)
          {
            ...
            if (debugLevel > 2)
              printPgnAnnoucement(scanReplyMachine, sizeof(scanReplyMachine), (char *)"Machine Reply");
          }
        }
      }
    ...
    {
      if (debugLevel > 3)
        printPgnAnnoucement(pgnData, len, (char *)"64 Section Data");
      ...
      if (debugLevel > 3)
      {
        ...
        Serial.print(states.leftSpeed);
        Serial.print(" Right speed: ");
        Serial.print(states.rightSpeed);
        Serial.println();
      }
    ...
    {
      if (debugLevel > 2)
        printPgnAnnoucement(pgnData, len, (char *)"Section Dims");
    ...
    {
      if (debugLevel > 2)
        printPgnAnnoucement(pgnData, len, (char *)"Machine Pin Config");
      ...
      if (config.pinFunction[24] == 16) {states.SecContOnBoardIsActive = false; Serial.println("AiO onboard section control is OFF");} 
      else{states.SecContOnBoardIsActive = true;Serial.println("AiO onboard section control is ON");} // set to true if the section control is on the AiO board, false if it is on a separate board (ie. in the implement)}
      
      if (tempFunction != config.pinFunction)
      { // compare, if different do stuff
        if (debugLevel > 2)
          printPinConfig();
        saveToEeprom();
    ...
    {
      if (debugLevel > 2)
        printPgnAnnoucement(pgnData, len, (char *)"Machine Config");
    ...
    {
      if (debugLevel > 3)
        printPgnAnnoucement(pgnData, len, (char *)"Machine Data");
      ...
      states.SecFromAOG[0] = pgnData[11]; // use 64 Sections PGN data instead, works properly with zones?
      states.SecFromAOG[1] = pgnData[12]; // i think zones has been fixed now in this PGN
      ...
      {
        Serial.print("\r\nuTurn: ");
        Serial.print(states.uTurn);
        Serial.print("\r\ngpsSpeed: ");
        Serial.print(pgnData[6]);
        Serial.print(" > ");
        Serial.print(states.gpsSpeed);
        Serial.print("\r\nhydLift(1:D 2:U): ");
        Serial.print(states.hydLift);
        Serial.print((states.hydLift == 1 ? ":DOWN" : states.hydLift == 2 ? ":UP"
                                                                          : ":OFF"));
        Serial.print("\r\ntramline(1:R 2:L): ");
        Serial.print(states.tramline);
        Serial.print((states.tramline == 1 ? ":RIGHT" : states.tramline == 2 ? ":LEFT"
                                                                             : ":OFF"));
        Serial.print("\r\ngeoStop(0:OK 1:STOP): ");
        Serial.print(states.geoStop);
        Serial.print((states.geoStop == 1 ? ":STOP!!!" : ":GOOD"));
        Serial.print("\r\nsec 1-8: ");
        printBinaryByteLSB(pgnData[11], 8);
        Serial.print(" sec 9-16: ");
        printBinaryByteLSB(pgnData[12], 8);
      }
      else if (debugLevel > 3)
      {
        Serial.print("\r\n0:");
        printBinaryByteLSB(states.SecFromAOG[0], 8);
        Serial.print(" 1:");
        printBinaryByteLSB(states.SecFromAOG[1], 8);
  ...
  void printPgnAnnoucement(uint8_t *_data, uint8_t _len, char *_pgnName)
  {
    ...
    for (uint8_t i = strlen(_pgnName); i < 20; i++)
      Serial.print(" "); // to align PGN data dump with all PGNs
    Serial.printf(" %2i Data>", _len);
    // Serial.print(millis());

    // Serial.print(" Data: ");
    for (byte i = 4; i < _len - 1; i++)
    { // -1 skips printing CRC
      Serial.printf("%3i", _data[i]);
  ...
  void loadFromEeprom()
  {
    ...
    if (EEread != EE_IDENT)
    { // check on first start and write EEPROM
  ...
  {
    Serial.print("\r\n- raiseTime: ");
    Serial.print(config.raiseTime);
    Serial.print("\r\n- lowerTime: ");
    Serial.print(config.lowerTime);
    Serial.print("\r\n- relayActiveHigh: ");
    Serial.print(config.isPinActiveHigh);
    Serial.print("\r\n- hydLiftEnable: ");
    Serial.print(config.hydLiftEnable);
    Serial.print("\r\n- user1: ");
    Serial.print(config.user1);
    Serial.print("\r\n- user2: ");
    Serial.print(config.user2);
    Serial.print("\r\n- user3: ");
    Serial.print(config.user3);
    Serial.print("\r\n- user4: ");
    Serial.print(config.user4);
  }
